import https from 'https';
import http from 'http';

/**
 * Script TypeScript para verificar se a meta tag "google-site-verification" foi adicionada com sucesso
 * Uso: npx ts-node verify-google-meta-tag.ts <url> [código-esperado]
 * Exemplo: npx ts-node verify-google-meta-tag.ts https://meusite.com.br
 * Exemplo com validação: npx ts-node verify-google-meta-tag.ts https://meusite.com.br "abc123xyz456"
 */

interface VerificationResult {
  success: boolean;
  statusCode: number;
  metaTags: string[];
  foundCode?: string;
  expectedCode?: string;
  message: string;
}

// Cores para output
const colors = {
  red: '\x1b[0;31m',
  green: '\x1b[0;32m',
  yellow: '\x1b[1;33m',
  blue: '\x1b[0;34m',
  reset: '\x1b[0m',
};

function log(color: keyof typeof colors, message: string): void {
  console.log(`${colors[color]}${message}${colors.reset}`);
}

async function fetchHTML(url: string): Promise<{ statusCode: number; html: string }> {
  return new Promise((resolve, reject) => {
    const protocol = url.startsWith('https') ? https : http;

    const req = protocol.get(
      url,
      {
        timeout: 10000,
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; GoogleVerificationChecker/1.0)',
        },
      },
      (res) => {
        let data = '';

        res.on('data', (chunk) => {
          data += chunk;
        });

        res.on('end', () => {
          resolve({ statusCode: res.statusCode || 200, html: data });
        });
      }
    );

    req.on('error', (err) => {
      reject(new Error(`Erro na requisição: ${err.message}`));
    });

    req.on('timeout', () => {
      req.destroy();
      reject(new Error('Timeout na requisição (10s)'));
    });
  });
}

async function verifyGoogleMetaTag(
  siteUrl: string,
  expectedCode?: string
): Promise<VerificationResult> {
  const timestamp = new Date().toLocaleString('pt-BR');

  log('blue', '========================================');
  log('blue', 'Verificador de Meta Tag Google');
  log('blue', '========================================');
  console.log(`Data/Hora: ${timestamp}`);
  console.log(`URL: ${siteUrl}`);
  if (expectedCode) {
    console.log(`Código esperado: ${expectedCode}`);
  }
  console.log('');

  try {
    // Passo 1: Fazer requisição HTTP
    log('yellow', '1. Fazendo requisição para o site...');
    const { statusCode, html } = await fetchHTML(siteUrl);

    if (statusCode !== 200) {
      log('red', `✗ Erro na requisição HTTP`);
      console.log(`Código HTTP: ${statusCode}`);
      return {
        success: false,
        statusCode,
        metaTags: [],
        message: `Erro HTTP ${statusCode}`,
      };
    }

    log('green', `✓ Requisição bem-sucedida (HTTP ${statusCode})`);
    console.log('');

    // Passo 2: Procurar pela meta tag
    log('yellow', '2. Procurando pela meta tag google-site-verification...');

    const metaTagRegex =
      /<meta[^>]*name=["']?google-site-verification["']?[^>]*>/gi;
    const metaTags = html.match(metaTagRegex) || [];

    if (metaTags.length === 0) {
      log('red', '✗ Meta tag google-site-verification NÃO encontrada');
      console.log('');
      log('yellow', 'Dicas:');
      console.log('- Verifique se o arquivo foi publicado corretamente no deploy');
      console.log('- Aguarde alguns minutos (cache pode estar ativo)');
      console.log('- Confirme que a meta tag está em _document.tsx ou pages/_document.tsx');
      console.log('- Use: curl -s https://seu-site.com.br | grep -i google-site-verification');
      console.log('');
      return {
        success: false,
        statusCode,
        metaTags: [],
        message: 'Meta tag não encontrada',
      };
    }

    log('green', '✓ Meta tag encontrada!');
    console.log('');

    // Passo 3: Exibir detalhes da meta tag
    log('yellow', '3. Detalhes da meta tag encontrada:');
    metaTags.forEach((tag, index) => {
      if (metaTags.length > 1) {
        console.log(`   Meta tag ${index + 1}:`);
      }
      console.log(`   ${tag}`);
    });
    console.log('');

    // Passo 4: Validar o código se fornecido
    if (expectedCode) {
      log('yellow', '4. Validando código...');

      const foundTag = metaTags.find((tag) => tag.includes(expectedCode));

      if (foundTag) {
        log('green', '✓ Código corresponde ao esperado');
        log('green', `Código encontrado: ${expectedCode}`);
        console.log('');
        log('blue', '========================================');
        log('green', '✓ Verificação concluída com sucesso!');
        log('blue', '========================================');
        console.log('');

        return {
          success: true,
          statusCode,
          metaTags,
          foundCode: expectedCode,
          expectedCode,
          message: 'Meta tag e código validados com sucesso',
        };
      } else {
        log('red', '✗ Código NÃO corresponde ao esperado');
        console.log(`Código esperado: ${expectedCode}`);

        const contentMatch = metaTags[0]?.match(/content=["']?([^"'>\s]+)["']?/i);
        if (contentMatch) {
          console.log(`Código encontrado: ${contentMatch[1]}`);
        }

        return {
          success: false,
          statusCode,
          metaTags,
          foundCode: contentMatch ? contentMatch[1] : undefined,
          expectedCode,
          message: 'Código não corresponde ao esperado',
        };
      }
    }

    console.log('');
    log('blue', '========================================');
    log('green', '✓ Verificação concluída com sucesso!');
    log('blue', '========================================');
    console.log('');

    return {
      success: true,
      statusCode,
      metaTags,
      message: 'Meta tag encontrada com sucesso',
    };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    log('red', `✗ Erro: ${errorMessage}`);
    return {
      success: false,
      statusCode: 0,
      metaTags: [],
      message: errorMessage,
    };
  }
}

// Validar argumentos e executar
const args = process.argv.slice(2);

if (args.length < 1) {
  log('red', 'Erro: URL do site é obrigatória');
  console.log('Uso: npx ts-node verify-google-meta-tag.ts <url-do-site> [código-esperado-opcional]');
  console.log('Exemplo: npx ts-node verify-google-meta-tag.ts https://meusite.com.br');
  console.log('Exemplo com validação: npx ts-node verify-google-meta-tag.ts https://meusite.com.br "abc123xyz456"');
  process.exit(1);
}

const siteUrl = args[0];
const expectedCode = args[1] || undefined;

verifyGoogleMetaTag(siteUrl, expectedCode).then((result) => {
  if (!result.success) {
    process.exit(1);
  }
});
